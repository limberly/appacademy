...
def harrison_ford
  ...
  Movie.joins(actors: :castings).where("actors.name = ? AND castings.ord != ?", "Harrison Ford", 1).distinct.select("movies.id", "movies.title")
end
...
def biggest_cast
  ...
  Movie.joins(:castings).group("movies.id", "movies.title").order("count(movies.id) DESC").limit(3).select("movies.id", "movies.title")
end
