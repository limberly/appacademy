...
describe SuperComputerPlayer do
  ...
  describe "#move" do
    it "chooses winning move if one is available" do
      # debugger
      expect(subject.move(winnable_game.board, :x)).to eq([0, 2])
    end

    it "can pick a winner that is two moves away" do
      move = subject.move(two_moves_to_victory_game.board, :x)
    ...
    it "blocks an opponent's winning move" do
      ...
      # debugger
      expect(subject.move(blockable_win_game.board, :x)).to eq([1, 2])
    end

    it "raises an error if it cannot find a winning or draw inducing move" do
      expect do
        subject.move(unwinnable_game.board, :x)
      end.to raise_error
    end
  end
end
